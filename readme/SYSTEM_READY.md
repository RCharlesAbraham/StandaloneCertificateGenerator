...
1. Go to: `http://localhost:3000/public/admin/login.php`
...
✅ **Frontend Pages:**
 - `public/login.php` - User login with sign-up link
 - `public/register.php` - Student/Staff registration
- `index.php` - Certificate generator (requires login)
 - `public/admin/login.php` - Admin login
 - `public/admin/panel.php` - Admin dashboard

✅ **Backend Handlers:**
 - `public/actions/login_process.php` - Authenticates users
 - `public/actions/register_process.php` - Creates new accounts
 - `public/actions/logout.php` - Logs out users
 - `public/actions/log_certificate.php` - Logs certificate generation
 - `public/admin/admin_login_process.php` - Authenticates admins
 - `public/admin/admin_api.php` - Provides admin panel data
...
### User Journey:
1. **Register** → `public/register.php` (create account)
2. **Login** → `public/login.php` (authenticate)
3. **Generate** → `index.php` (create certificates)
4. **Download** → PDF files automatically
5. **Logout** → Session ends, redirect to login

### Admin Journey:
1. **Login** → `public/admin/login.php` (use default credentials)
2. **Monitor** → `public/admin/panel.php` (view everything)
